...
function deleteExpense () {
    ...
    prompt.get(['name','expense'], (err,user) => {
        ...
        .then(res=> res === null ?
            ...
            : 
            User.findOne({name:user.name}) 
            .then(data => { index = data.expensename.indexOf(user.expense)
            User.findOneAndUpdate({name:user.name}, {$pull:{expensename:user.expense}})
            .then(data=>data)
            User.findOneAndUpdate({name:user.name}, { expense: {$slice:index}})
